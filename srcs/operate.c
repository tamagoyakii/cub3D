...
void	move_ws(t_vec *v, t_cub *c, int opt)
{
	...
	des_x = (int)(v->pos_x + v->dir_x * (double)MOVE_SPEED / 100 * opt);
	des_y = (int)(v->pos_y + v->dir_y * (double)MOVE_SPEED / 100 * opt);
...
void	move_ad(t_vec *v, t_cub *c, int opt)
{
	...
	des_x = (int)(v->pos_x + v->pln_x * (double)MOVE_SPEED / 100 * opt);
	des_y = (int)(v->pos_y + v->pln_y * (double)MOVE_SPEED / 100 * opt);
	if (c->map[(int)v->pos_y][des_x] == '0')
		v->pos_x = des_x;
	if (c->map[des_y][(int)v->pos_x] == '0')
		v->pos_y = des_y;
